...
export async function PUT(req) {
    ...
    try {
        ...
        const dbStatus = statusMap[novoStatus];
        if (!dbStatus) return NextResponse.json({ error: "Status inválido" }, { status: 400 });

        const { data: updatedVenda, error: updateError } = await supabase
            .from("venda")
            .update({ status: dbStatus, atualizada_em: new Date().toISOString() })
            .eq("idvenda", pedidoId)
            .select("online")
            .single();
        ...
        if (updatedVenda.online && formatado) {
