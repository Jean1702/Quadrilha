...
export default function PedidoFisicoPage({ vendas, admindata, produtos }) {
  ...
  const produtoSelecionado = produtosLoja.find(p => p.idproduto === parseInt(produtoAtual));

  const quantidadeJaAdicionada = itensPedido
    .filter(item => item.id === parseInt(produtoAtual))
    .reduce((acc, item) => acc + item.quantidade, 0);

  const estoqueDisponivel = produtoSelecionado
    ? Math.max(0, produtoSelecionado.estoque - quantidadeJaAdicionada)
    : 0;
  ...
  const handleAdicionarProduto = () => {
    ...
    if (produtoEncontrado) {
      if (parseInt(quantidadeAtual) > estoqueDisponivel) {
        alert(`Você só pode adicionar mais ${estoqueDisponivel} unidade(s) deste produto.`);
        return;
      }
  ...
  const pedidosFiltrados = vendasatual.filter((pedido) => {
    ...
    const nomeCliente = pedido.nome_cliente ? pedido.nome_cliente.toLowerCase() : "";


    const matchTexto =
      nomesProdutos.includes(filtroTexto.toLowerCase()) ||
      nomeCliente.includes(filtroTexto.toLowerCase()) ||
  ...
  const handleRegistrarClick = () => {
    // Validação para garantir que o admin não esqueça o nome do cliente
    if (!form.metodo || !form.nomeCliente.trim() || itensPedido.length === 0) {
      alert("Por favor, preencha o nome do cliente, a forma de pagamento e adicione itens.");
      return;
    }
  ...
  const confirmRegistrar = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const novosProdutos = produtosLoja.map(p => {
          const itemComprado = itensPedido.find(i => i.id === p.idproduto);
          if (itemComprado) {
            return { ...p, estoque: Math.max(0, p.estoque - itemComprado.quantidade) };
          }
          return p;
        });

        const { data: novaVendaDb } = await response.json();
        if (novaVendaDb) {
          setVendasatual(prev => [novaVendaDb, ...prev]);
        }

        setProdutosLoja(novosProdutos);
        setItensPedido([]);
        setForm(prev => ({ ...prev, metodo: "", nomeCliente: "" }));
    ...
    <div className="min-h-screen pb-28">
      ...
      <div className="max-w-3xl mx-auto w-full">
        <div className="bg-(--surface) p-6 rounded-[20px] shadow-lg mb-6">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (num > estoqueDisponivel) {
                    setQuantidadeAtual(estoqueDisponivel);
                  } else {
                ...
                onBlur={() => {
                  if (quantidadeAtual === "" || quantidadeAtual < 1) {
                    setQuantidadeAtual(estoqueDisponivel > 0 ? 1 : 0);
                  }
                }}
              />
            </div>
          ...
          <input
              className="col-span-1 sm:col-span-2 p-3 rounded-full border bg-(--bg) border-gray-300 focus:border-[#D97016] focus:ring-2 focus:ring-[#D97016]/30 outline-none transition duration-300 shadow-sm placeholder:opacity-60"
              type="text"
