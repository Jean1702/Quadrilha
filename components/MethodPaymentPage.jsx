...
export default function MethodPaymentPage() {
    ...
    const isFormIncomplete =
        (isCardMethod && (!cardData.number || !cardData.name || !cardData.expiry || !cardData.security_code || !cardData.CPF || !cardData.email)) ||
        (isPixMethod && (!cardData.CPF || !cardData.email));
    ...
    const handleFinalizarCompra = async () => {
        ...
        try {
            ...
            if (paymentMethod !== 'pix') {
                ...
                const [mes, ano] = cardData.expiry && cardData.expiry.includes('/')
                    ? cardData.expiry.split('/')
                    : ['', ''];

                // 2. Só tenta ler o .length se 'ano' for uma string válida
                ...
                <section className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm sticky top-8">
                        ...
                        {paymentOptions.map((option) => {
                            ...
                            return (
                                <div key={option.id} className="overflow-hidden">
                                    ...
                                    <div className={`transition-all duration-300 ease-in-out ${isSelected ? 'max-h-[800px] opacity-100 mt-4' : 'max-h-0 opacity-0'}`}>
                                        <div className="p-4 bg-gray-50 rounded-2xl space-y-3 border border-gray-100">
                                                ...
                                                <>
                                                    <div>
                                                        ...
                                                        <input
                                                            ...
                                                            className="w-full bg-white text-gray-900 border border-gray-200 rounded-xl p-3 text-sm placeholder:text-gray-400 focus:ring-2 focus:ring-orange-500 outline-none transition-all"
                                            ...
                                            <div className='flex items-start gap-2 mt-4 bg-orange-50/50 p-2 rounded-lg border border-orange-100'>
                                                ...
                                                <p className='text-[10px] text-gray-500 leading-tight flex-1'>
                                                    ...
                                                    Os dados informados acima (incluindo CPF e E-mail) serão utilizados apenas e unicamente para processar a efetuação do pagamento.
                                                </p>
                                            </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100">
                        ...
                        <button
                            ...
                            disabled={isLoading || carrinho.length === 0 || isFormIncomplete}
